...
from ibpykernel.jsonutil import json_clean
from traitlets import Any, Enum, Instance, List, Type, default
from ibpykernel.ipkernel import IPythonKernel
from ibpykernel.zmqshell import ZMQInteractiveShell
...
class InProcessKernel(IPythonKernel):
    ...
    frontends = List(
        Instance('ibpykernel.inprocess.client.InProcessKernelClient',
                 allow_none=True)
    )
...
class InProcessInteractiveShell(ZMQInteractiveShell):
    ...
    kernel = Instance('ibpykernel.inprocess.ipkernel.InProcessKernel',
                      allow_none=True)
    ...
    def enable_gui(self, gui=None):
        ...
        from ibpykernel.eventloops import enable_gui
        if not gui:
            gui = self.kernel.gui
